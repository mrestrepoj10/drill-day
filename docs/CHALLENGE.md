...
2. Open the **withheld** badge in the navbar before calling anything: it names the tools this stage has switched off and what withholding each one teaches.
...
- Instructional allow lists demonstrate that tool availability can be part of the simulation—not merely an API wrapper. A navbar badge states the current stage's allow list before the agent tests it, so the guardrail is legible without a refusal having to happen first.
...
- Navbar allow-list badge: the withheld tools and their reasons, stated before the agent tests them.
...
- [ ] Flagship CTA, allow-list badge, guardrail, hint, agent attempt and pinned note, near miss, correct isolation, replay, and agent-authored drill are smoke-tested.
