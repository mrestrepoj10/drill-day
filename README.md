...
Every tool has a constrained JSON Schema, a human-readable title and description, truthful read-only annotations, and an async executor. Mission-level allow lists enforce instructional guardrails, and the navbar's **Agent tools** badge lists every site tool, naming the ones the current stage withholds before the agent tries one. An agent can inspect a valve while being refused permission to locate it.
