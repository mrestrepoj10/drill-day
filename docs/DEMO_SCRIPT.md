...
The same happens with no drill loaded at all: take the shift-handover prompt from the start card's **Brief me** tab and paste it and the agent walks the building in first person while it briefs you. The panel gets out of the way for the duration and the floor plan comes out on the model, so the shot is the building and where you are in it. A whole briefing is one call — the notes appear together and the camera walks the set while the agent writes its handover — so the walkthrough plays instead of stopping between every stop to wait on a model. The page reacts on its first call: the panel steps aside, the plan comes up, and the route it is reading lights through the building — there is no dead minute while it thinks. When the agent goes quiet the camera flies back to the view it set off from, with every note it pinned visible on the model at once — that is the frame to end the section on. The plan stays up with a **Walk the notes** control under it, so you can step back through the briefing at your own pace, in first person, one note at a time.
